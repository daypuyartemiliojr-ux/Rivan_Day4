...
| HR         | 192.168.128.0       | 192.168.130.151 /20 | 
...
conf t
 ...
 int g3
  ...
  exit
 int g4
  ip add 10.51.1.11 255.255.255.0
  no shut
  exit
 !
 ...
 !
 username admin privilege 51 secret pass
 ip http server
...
ip route add 10.0.0.0/8 via 10.51.1.4 dev ens256
ip route add 200.0.0.0/24 via 10.51.1.4 dev ens256
...
__Make Sure EDGE-51 is Configured__
...
### Task 1: Prevent traffic only from VLAN 100 (CUCM) to reach UTM-PH (10.51.1.11)
~~~
!@UTM-PH
config t
...
### Task 3: Allow the PC (10.51.1.10) to access HTTP, SSH, and ICMP of UTM-PH (10.51.1.11)
...
!@UTM-PH
config t
 ip host www.bet11.com 10.51.1.11
 service finger
 ...
 telephony-service
  ...
  max-ephones 5
  max-dn 20
  ip source-address 10.51.1.11 port 2000
  exit
 voice service voip
 ...
 voice register global
  mode cme
  source-address 10.51.1.11 port 5060
  max-dn 12
  max-pool 12
...
- Allow Pings destined for your PC (10.51.1.10)  
...
config t
 ip access-list extended FWP5
  permit  icmp  any  host 10.51.1.10             log 
...
conf t
 ...
 ip access-list extended NAT
  deny ip 11.11.11.96 0.0.0.31  21.21.21.208 0.0.0.51
...
| SERVER      | INSIDE PORT | OUTSIDE IP     | OUTSIDE PORT |
| ---         | ---         | ---            | ---          |
| CoreTAAS    | 23          | 200.0.0.51 | 2023         |
| CoreBABA    | 23          | 200.0.0.51 | 4023         |
| CUCM        | 23          | 200.0.0.51 | 8020         |
...
conf t
 ...
 ip access-list extended NAT-POLICY
  permit ip 10.51.0.0  0.0.255.255  any
  exit
 !
 !
...
!@EDGE-51
conf t
 ip nat inside source static tcp  10.51.1.4  23  200.0.0.51  4023
 !
 ip nat inside source static tcp  10.51.1.4  __  __.__.__.__     __
 ip nat inside source static tcp  10.51.1.4  __  __.__.__.__     __
 end
~~~
...
!@EDGE-51
conf t
 ip nat inside source static tcp  10.51.1.11  6969  200.0.0.51  3306
 end
~~~
...
conf t
 ...
 ip access-list extended NAT
  deny ip 11.11.11.0 0.0.0.31  21.21.21.208 0.0.0.51
...
conf t
 ...
 !
 ip access-list extended NAT
  deny ip 21.21.21.208 0.0.0.51  11.11.11.96 0.0.0.31
...
!@UTM-PH
conf t
 username rivan privilege 51 secret pass
 ip ssh pubkey-chain
  username rivan
   key-string
